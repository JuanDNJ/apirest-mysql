    ...
    try { // Utilizo try catch para capturar errores
        ...
        switch (field) {
            ...
            case "password": // si el campo es password
                ...
                if (value.match(/[^a-zA-Z0-9çÇ@€]/g)) return { message: "The password can only contain letters and numbers and characters(€, @, ç, Ç )." } // si la contraseña contiene caracteres especiales, enviar error
            ...
            case "user_handler": // si el campo es user_handler
                if (value.length > 20) return { message: "The user handler cannot be longer than 20 characters." } // si el user handler es mayor a 15 caracteres, enviar error
                if (value.length < 6) return { message: "The user handler cannot be less than 3 characters." } // si el user handler es menor a 8 caracteres, enviar error
                if(value.match(/[^a-zA-Z0-9çÇ@€]/g)) return { message: "The user handler can only contain letters and numbers and characters (€, @, ç, Ç )." } // si el user handler contiene caracteres especiales, enviar error
                return { message: "The user handler is valid." } // si el user handler es válido, enviar respuesta
                break;
            case "age": // si el campo es edad (age)
                if (value < 18) return { message: "You must be over 18 years old." } // si la edad es menor a 18 años, enviar error
                if (value > 100) return { message: "You must be under 100 years old." } // si la edad es mayor a 100 años, enviar error
                if(value.match(/[^0-9]/g)) return { message: "The age can only contain numbers." } // si la edad contiene caracteres especiales, enviar error
                return { message: "The age is valid." } // si la edad es válida, enviar respuesta
                break;
            case "birthday": // si el campo es fecha de nacimiento (birthday)
                ...
                if (age < 18) return { message: "You must be over 18 years old." } // si la edad es menor a 18 años, enviar error
                return { message: "The birthday is valid." } // si la fecha de nacimiento es válida, enviar respuesta
                break;
            default:
